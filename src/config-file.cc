  ...
  while (std::getline(file, line)) {
    ...
    if (std::regex_match(line, match, sectionRegex)) {
      if (hasSection(match[1].str()))
        throw std::runtime_error(
            std::string{filename} + ": line " + std::to_string(lineNumber) +
            ": duplicate definition of section '" + match[1].str() + "'");
    ...
    /* A key value pair? */
      ...
      if (hasProperty(currentSection, match[1].str()))
        throw std::runtime_error(
            std::string{filename} + ": line " + std::to_string(lineNumber) +
            ": duplicate definition of '" + match[1].str() + "'");
    ...
    /* An empty line, perhaps? Otherwise: give up. */
      ...
      std::cerr << "warning: " << filename << ": cannot parse line "
                << lineNumber << ", ignoring." << std::endl;
    }
  }
...
{
  return sections.end() !=
         std::find(sections.begin(), sections.end(), sectionName);
}
...
{
  return std::any_of(
      properties.begin(), properties.end(), [&](const auto& kvp) {
        return kvp.first.first == sectionName && kvp.first.second == keyName;
      });
}
