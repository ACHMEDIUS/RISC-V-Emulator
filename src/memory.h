...
public:
  ...
  Memory(const std::string& name, std::byte* data, const MemAddress base,
...
private:
  ...
  template <typename T> T readData(MemAddress addr);
  template <typename T> void writeData(MemAddress addr, T value);
};
